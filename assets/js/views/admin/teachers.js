const AdminTeachers = (() => {
  ...
  async function render() {
    ...
    page.innerHTML = `
      ...
      <div class="filters-bar mb-20">
        ...
        <select class="form-select" onchange="AdminTeachers.filterStatus(this.value)">
  ...
  async function impersonate(id) {
    try { await Auth.impersonate(id); window.location.hash = '#/teacher/dashboard'; window.location.reload(); }
    catch (err) { Toast.error(err.message); }
  }
