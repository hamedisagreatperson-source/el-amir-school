...
exports.getPayments = async (req, res, next) => {
  try {
    const { month = getCurrentMonth(), course_id, status, search, page = 1 } = req.query;
    const limit = 20;
    const offset = (page - 1) * limit;
...
exports.createPayment = async (req, res, next) => {
  try {
    const { student_id, course_id, month, amount, status = 'unpaid' } = req.body;
    ...
    if (error) throw error;
    await logAudit(req, 'create_payment', 'payment', data.id, { month, amount, status });
    res.json({ payment: data });
  } catch (err) {
    next(err);
  }
...
exports.generatePayments = async (req, res, next) => {
  try {
    ...
    const inserts = students.map(s => ({
      ...
      month,
      amount: s.courses ? Number(s.courses.price) : 0,
      status: 'unpaid'
    }));

    const { data, error } = await supabase
      .from('payments')
      .upsert(inserts, { onConflict: 'student_id,course_id,month', ignoreDuplicates: true })
      .select();

    if (error) throw error;
    await logAudit(req, 'generate_payments', 'payment', null, { month, count: (data || []).length });
